...
phase 1: input
  ...
  collects address or coordinates, room type, area, height, room/pano image, resident profile, constraints, and comfort note through the chat-led setup
...
The Phase 2 room viewer, Phase 3 room-retrofit component viewer, KG viewer, validation view, and final report view are generated or served by backend exporters. React displays them as embedded static pages or linked views.
...
The phase 3 visualization bar renders the dynamically ranked top three retrofit packages for the current diagnosis and constraints. The `room` view remains the original spatial room viewer. The separate `components` view loads `room_3d_component_view.html`, now the full textured 3D retrofit preview with tested components, before/after effects, benchmark status, and clickable wall diagnosis. The final report uses this 3D preview instead of a separate perspective-image workflow.
...
The site-context visual phase is deactivated. Risk-map data remains backend-only and is passed to diagnosis as numerical context until reliable raster cells are available.
